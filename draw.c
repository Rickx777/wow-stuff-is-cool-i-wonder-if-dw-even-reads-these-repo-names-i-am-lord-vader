...
void main(){
  ...
  struct matrix *polygons = new_matrix(4,3);
  struct matrix *rot = make_rotX(M_PI/180 * 50);
  //matrix_mult(make_rotY(M_PI/180 * 30),rot);
  //matrix_mult(make_translate(0,150,0),rot);
  
  //add_box(polygons,250,250,0,50,90,75);
  //add_sphere(polygons,250,250,0,200,0.1);
  //add_torus(polygons,250,250,0,50,100,0.1);
...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
 ...
 for (point=0; point < polygons->lastcol-2; point+=3){
   int Ax = (int)(polygons->m[0][point + 1] - polygons->m[0][point]);
   int By = (int)(polygons->m[1][point + 2] - polygons->m[1][point]);
   int Bx = (int)(polygons->m[1][point + 1] - polygons->m[1][point]);
   int Ay = (int)(polygons->m[0][point + 2] - polygons->m[0][point]);
   int Nz = Ax * By - Bx * Ay;
...
void add_box( struct matrix * edges,
  ...
  add_polygon(edges, x0, y0, z0, x0, y0, z1, x0, y1, z1); //left face
  add_polygon(edges, x0, y0, z0, x0, y1, z1, x0, y1, z0);

  add_polygon(edges, x0, y0, z0, x1, y1, z0, x1, y0, z0); //front face
  add_polygon(edges, x0, y0, z0, x0, y1, z0, x1, y1, z0);

  add_polygon(edges, x1, y0, z0, x1, y1, z1, x1, y0, z1); //right face
  add_polygon(edges, x1, y0, z0, x1, y1, z0, x1, y1, z1);
  
  add_polygon(edges, x0, y0, z1, x1, y1, z1, x0, y1, z1); //back face
  add_polygon(edges, x0, y0, z1, x1, y1, z1, x0, y1, z1);
  
  add_polygon(edges, x0, y0, z0, x1, y0, z0, x1, y0, z1); //top face
  add_polygon(edges, x0, y0, z0, x1, y0, z1, x0, y0, z1);
    
  add_polygon(edges, x0, y1, z0, x1, y1, z1, x1, y1, z0); //bottom face
  ...
  for ( lat = latStart; lat < latStop; lat++ ) {
    for ( longt = longStart; longt < longStop; longt++ ) {
      ...
      add_polygon( edges, points->m[0][index],
		   points->m[1][index],
		   points->m[2][index],
		   points->m[0][(index + num_steps) % points->lastcol],
		   points->m[1][(index + num_steps) % points->lastcol],
		   points->m[2][(index + num_steps) % points->lastcol],
		   points->m[0][(index + num_steps + 1) % points->lastcol],
		   points->m[1][(index + num_steps + 1) % points->lastcol],
		   points->m[2][(index + num_steps + 1) % points->lastcol]);
      add_polygon( edges, points->m[0][(index + 1) % points->lastcol],
